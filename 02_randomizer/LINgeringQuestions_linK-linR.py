#LINgeringQuestions -- Kevin Lin, Ricky Lin
#SoftDev1 pd06
#K02 NO-body expects the Spanish Inquisition!
#2018-09-11

#uses random library for randomization
import random
...
#puts all the teams into one array to pick from
teams = ['w', 'm', 'x']

def randomizer():
    #choose a team or go random
    pick = input("Pick w, m, x or type something else for random \n")
    if pick in KREWES.keys():
        print (random.choice(KREWES[pick]))
    else:
        print(random.choice(KREWES[random.choice(teams)]));
